...
- **Q Score names two different things — settle with the team BEFORE Phase 8 is planned.** `docs/roadmap-online-sourcing-and-renewal.md` §3 defines a per-employee compliance rollup (`100 x Coverage x (0.75 + 0.25 x Quality)`); a teammate's breakdown defines a per-attempt performance score (`raw % x difficulty x consistency`) written to a `q_score` column in `POST /quiz/submit`. Different grain, subject, and consumer. Reconcilable by renaming — the per-attempt number is a natural input to the rollup's `Quality` term — but shipping both under one name makes "Q Score 82" ambiguous. If the team picks the per-attempt definition, QSCORE-01, QSCORE-02, and QSCORE-04 all change.
